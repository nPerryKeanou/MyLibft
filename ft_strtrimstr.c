...
size_t ft_endingtrim(const char *paramS1, const char *paramSet){
	...
	j = 1;
	booleTrue = 1;
	len_s1= ft_strlen_const(paramS1);//longueur total de la chaine, donc ex : 10 char.
...
//	while(paramS1[end - len_set - 1] == paramSet[i]){
	while(booleTrue == 1){
		...
		//ici dans l'index de paramS1, on doit avoir comme valeur, len_s1 - (len_set * end)
...
char    *ft_strtrimstr(const char *s1, const char    *set){
	...
	printf("---- 4 -----\n");
	if(s1[lens1 - ft_strlen_const(set)] != set[i]){
		printf("---- 5 -----\n");
	...
	printf("lenS1 --> %zu\n", ft_strlen_const(s1));
	printf("len_set --> %zu\n", ft_strlen_const(set));
	printf("lenS1 - start - end + 1 ==> %zu\n", lens1 - start - end + 1);
	printf("---- 7-----\n");
	str = (char *)malloc(lens1 - start - end + 1 * sizeof(char));
	if(str == NULL){
		return(NULL);
	}
	printf("---- 8 -----\n");
	while(s1[i] != '\0' && i < lens1 - start - end - 1){
		printf("---- 9 -----\n");
		str[i] = s1[start];
		printf("s1[%zu] --> %c && s1[%zu] --> %c\n",i,s1[i], start ,s1[start]);
		i++;
		start++; 
	}
	printf("---- 10 -----\n");
